...
const createEmbeddedPaymentLink = async (req, res) => {
  try {
    ...
    if (!bookingRequest) {
      return res
        .status(404)
        .json({ error: 1, message: "Booking request not found" });
    }
...
const receivePayment = async (req, res) => {
  try {
    ...
    if (data.data && data.data.orderCode) {
        ...
        if (order.bookingId) {
          ...
          if (bookingRequest) {
            if (bookingRequest.status === "Completed") {
              ...
              console.log(
                `Booking ${order.bookingId} has status ${bookingRequest.status}, skipping update.`
              );
            }
          }
        }
      ...
      return res
        .status(200)
        .json({ error: 0, message: "Order updated successfully", order });
    }
